...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  py::class_<ServerT>(m, "Server")
    ...
    .def("enable_sampling_support", [](ServerT& server,
      ...
      // This enables to use the sampling support from the PyTorch bindings.
  ...
  py::class_<WorkerT>(m, "Worker")
    .def(py::init([](const int app_id, const int customer_id, ServerT& server) { // DEPRECATED
      ALOG("[DEPRECATED] You are using the adapm.Worker(app_id, customer_id, server) constructor to set up a worker, which is deprecated. Please switch to lapse.Worker(customer_id, server). No app_id is needed.");
